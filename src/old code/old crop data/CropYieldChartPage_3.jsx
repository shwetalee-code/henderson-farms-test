...
function CropYieldChartPage() {
  ...
  varieties.forEach(v => {
    const row = grouped[v];
    if (row['2022'] != null) { trace2022.x.push(v); trace2022.y.push(row['2022']); }
    if (row['2023'] != null) { trace2023.x.push(v); trace2023.y.push(row['2023']); }
    if (row['2024'] != null) { trace2024.x.push(v); trace2024.y.push(row['2024']); }
  });

  const getAverageTurnout = (yearColumn) => {
    const valid = turnoutData.filter(row => selectedOrchards.includes(row['Orchard Code']) && !isNaN(+row[yearColumn]));
    const avg = valid.reduce((sum, row) => sum + +row[yearColumn], 0) / (valid.length || 1);
    return valid.length ? `${(avg * 100).toFixed(1)}%` : 'N/A';
  };
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '30px', marginBottom: '20px' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <select multiple value={selectedOrchards} onChange={handleDropdownChange} style={{ width: '250px', marginTop: '10px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', color: 'black', fontWeight: 'bold', fontSize: '16px' }}>
          <div>2022 Turnout: {getAverageTurnout('Sum of 2022 Turn Out %')}</div>
          <div>2023 Turnout: {getAverageTurnout('Sum of 2023 Turnout')}</div>
          <div>2024 Turnout: {getAverageTurnout('Sum of 2024 Turnout %')}</div>
        </div>
      </div>
      ...
      <Plot
        data={[{
          ...
          y: [
            filtered.reduce((sum, d) => sum + (+d['Sum of 2022 Incoming'] || 0), 0),
            filtered.reduce((sum, d) => sum + (+d['Sum of 2022 USDA Mt Lbs'] || 0), 0),
            filtered.reduce((sum, d) => sum + (+d['Sum of 2023 Incoming'] || 0), 0),
            filtered.reduce((sum, d) => sum + (+d['Sum of 2023 USDA Mt Lbs'] || 0), 0),
            filtered.reduce((sum, d) => sum + (+d['Sum of 2024 Incoming'] || 0), 0),
            filtered.reduce((sum, d) => sum + (+d['Sum of 2024 USDA Mt Lbs'] || 0), 0)
          ],
          type: 'bar',
