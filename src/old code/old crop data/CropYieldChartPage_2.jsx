...
function CropYieldChartPage() {
  ...
  const percent = (usdaKey, incomingKey) => {
    const usda = sumMetric(usdaKey);
    const incoming = sumMetric(incomingKey);
    return incoming > 0 ? `${((usda / incoming) * 100).toFixed(1)}%` : 'N/A';
  };
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '30px', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', color: 'black', fontWeight: 'bold', fontSize: '16px' }}>
          <div>2022 Turnout: {percent('Sum of 2022 USDA Mt Lbs', 'Sum of 2022 Incoming')}</div>
          <div>2023 Turnout: {percent('Sum of 2023 USDA Mt Lbs', 'Sum of 2023 Incoming')}</div>
          <div>2024 Turnout: {percent('Sum of 2024 USDA Mt Lbs', 'Sum of 2024 Incoming')}</div>
        </div>
      </div>
