  ...
  useEffect(() => {
    Papa.parse('/finance fertilizer data.csv', {
      ...
      complete: (results) => {
        ...
        const withYear = cleaned.map(d => ({ ...d, Year: new Date(d['Date']).getFullYear().toString() }));
        setData(withYear);
        setFiltered(withYear);
        setItems([...new Set(withYear.map(d => d['Item']))].sort());
        setNames([...new Set(withYear.map(d => d['Name']))].sort());
        setYears([...new Set(withYear.map(d => d['Year']))].sort());
        setSelectedItems([...new Set(withYear.map(d => d['Item']))]);
        setSelectedNames([...new Set(withYear.map(d => d['Name']))]);
        setSelectedYears([...new Set(withYear.map(d => d['Year']))]);
      }
    });
  }, []);
