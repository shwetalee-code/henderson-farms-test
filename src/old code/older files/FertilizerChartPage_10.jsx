...
function FertilizerChartPage() {
  ...
  const fertilizerSet = [...new Set(sortedData.map(d => d.Fertilizer))];
  const traces = fertilizerSet.map(fert => ({
    x: xLabels,
    y: sortedData.filter(d => d.Fertilizer === fert).map(d => +d['Average of Sales Price']),
    name: fert,
    type: 'bar',
    yaxis: 'y1'
  }));
